...
function selectInternalPaths(paths) {
  const selected = {};
  for (const [path, pathItem] of Object.entries(paths)) {
    ...
    for (const [key, operation] of Object.entries(pathItem)) {
      ...
      if (!Array.isArray(audiences) || audiences.length !== 1 || audiences[0] !== "internal") {
...
function renderClient(document) {
  ...
  return [
    ...
    `export const internalOperations = ${JSON.stringify(operations, null, 2)} as const;`,
    "",
...
export async function renderArtifacts(root) {
  ...
  const internalPaths = selectInternalPaths(bundle.paths);
  const internal = {
    ...bundle,
  ...
  const artifacts = new Map([
    ...
    ["packages/api-client/src/index.ts", renderClient(internal)],
  ]);

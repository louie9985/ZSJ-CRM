      ...
      <Sidebar.Header>
        <div className="flex items-center gap-3 px-1 py-1">
          ...
          <div className="flex min-w-0 flex-col" data-sidebar="label">
            <span className="text-foreground text-sm font-medium leading-tight">Fred Palmer</span>
            <span className="text-muted text-xs font-medium leading-tight">fred@heroui.dev</span>
          </div>
        </div>
      </Sidebar.Header>
      <Sidebar.Content>
        <Sidebar.Group>
          <Sidebar.Menu aria-label="财务导航">
            {NAV_ITEMS.map((item) => (
              <SidebarNavItem
                key={item.href}
                basePath={basePath}

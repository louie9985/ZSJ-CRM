...
- `crm-permission-catalog.v1.json` declares the ADR-0033-confirmed top-level `crm.application:access` capability only. The fixed `crm.application-user` role bundle and Assignment-bound Grants remain in Authorization Policy v2 snapshots, never in the catalog.

...
import { createFormSchemaService, createMemoryFormSchemaStore, type FormAudit } from "@ai-crm/crm-form-schema";
import { createTaskCenter, InMemoryTaskCenterStore, type TaskAudit, type TaskLifecycleEvent } from "@ai-crm/crm-task-center";
...
describe("walking skeleton evidence chain", () => {
  it("keeps form release, stable FileReference, task completion and durable trace evidence together", async () => {
    ...
    await form.saveDraft({ ...metadata(), definitionId: "crm.synthetic.evidence", expectedRevision: 0, ownerModule: "crm.synthetic", jsonSchema: { $schema: "https://json-schema.org/draft/2020-12/schema", type: "object", additionalProperties: false, required: ["synthetic_value"], properties: { synthetic_value: { type: "string", minLength: 1 } } }, uiSchema: { fields: [{ component: "input", field: "synthetic_value", order: 1 }], layout: "vertical", version: 1 } });
    await form.publish({ ...metadata(), definitionId: "crm.synthetic.evidence", expectedRevision: 1 });
    await form.setReleaseActive({ ...metadata(), active: true, definitionId: "crm.synthetic.evidence", releaseVersion: 1 });
    const submission = { synthetic_value: fileReference.fileId };
    await expect(form.validateSubmission({ actor: formActor, data: submission, definitionId: "crm.synthetic.evidence", releaseVersion: 1 })).resolves.toMatchObject({ valid: true, reference: { definitionId: "crm.synthetic.evidence", releaseVersion: 1, version: 1 } });
    ...
    const event: TaskLifecycleEvent = { assigneeReference: actor.assignmentId, deepLink: { appId: "crm.synthetic", routeId: "crm.synthetic.detail" }, eventId: "00000000-0000-4000-8000-000000000302", occurredAt: at, sourceTaskId: "task.synthetic.evidence", sourceType: "crm.synthetic", sourceVersion: 1, status: "open" };
  ...
  it("rejects an expired form release before task completion can be attempted", async () => {
    ...
    await form.saveDraft({ ...metadata, definitionId: "crm.synthetic.expiring", expectedRevision: 0, ownerModule: "crm.synthetic", jsonSchema: { $schema: "https://json-schema.org/draft/2020-12/schema", type: "object", properties: {}, additionalProperties: false }, uiSchema: { fields: [], layout: "vertical", version: 1 } });
    await form.publish({ ...metadata, operationId: randomUUID(), definitionId: "crm.synthetic.expiring", expectedRevision: 1 });
    await form.setReleaseActive({ ...metadata, operationId: randomUUID(), active: false, definitionId: "crm.synthetic.expiring", releaseVersion: 1 });
    await expect(form.validateSubmission({ actor, data: {}, definitionId: "crm.synthetic.expiring", releaseVersion: 1 })).rejects.toMatchObject({ code: "form_not_found" });
  });
});

...
This directory is reserved for the business-neutral Walking Skeleton and later approved business workflows. The external FileReference/Trace bridge and the real isolated Task Projection Worker have successful executable evidence. The authenticated browser session still does not invoke the Task completion API that starts the durable Worker chain, so the main E2E must not be reported as complete.

...
- 所有成功、失败、错误和警告类操作提醒统一使用 Ant Design `Notification`，固定在页面右上方并显示在 Modal、Drawer 等弹出层之上；不得使用 `message` 或结果型 `Alert` 代替。表单字段校验、页面级 `Result`、加载状态以及非结果性的说明信息不属于操作提醒。
...
- 所有 `/crm/*` 页面必须位于唯一 `WorkbenchShell` 路由树内。岗位内容组件不得复制顶栏、一级导航、公共二级导航、账号菜单或全局布局样式。

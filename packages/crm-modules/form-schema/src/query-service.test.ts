...
const release = { active: true, content_digest: "a".repeat(64), definition_id: "crm.synthetic.form", json_schema: { $schema: "https://json-schema.org/draft/2020-12/schema", additionalProperties: false, properties: { synthetic_value: { type: "string" } }, required: ["synthetic_value"], type: "object" }, owner_module: "crm.synthetic", published_at: "2026-07-28T00:00:00.000Z", release_version: 1, ui_schema: { fields: [{ component: "input", field: "synthetic_value", order: 1 }], layout: "vertical", version: 1 } };
...
describe("createPrismaFormSchemaQueryService", () => {
  it("authorizes an exact release with the explicit subject before PostgreSQL", async () => {
    ...
    await expect(service.getRelease({ context: requestContext, definitionId: "crm.synthetic.form", releaseVersion: 1 })).resolves.toMatchObject({ releaseVersion: 1 });
    expect(authorize).toHaveBeenCalledWith({ action: "read", actor: requestContext.actor, definitionId: "crm.synthetic.form", permission: { action: "read", code: "crm.form-schema.form-release:read", resource: "crm.form-schema.form-release" }, releaseVersion: 1, subject: requestContext.subject, traceId: requestContext.traceId });
  ...
  it("validates against the exact immutable release without persisting data", async () => {
    ...
    await expect(service.validateSubmission({ context: requestContext, data: { synthetic_value: "ok" }, definitionId: "crm.synthetic.form", releaseVersion: 1 })).resolves.toMatchObject({ valid: true });
    await expect(service.validateSubmission({ context: requestContext, data: {}, definitionId: "crm.synthetic.form", releaseVersion: 1 })).resolves.toMatchObject({ valid: false });
  });
  it("does not accept an inactive release for new validation",async()=>{const execute:FormPersistenceRuntime["execute"]=vi.fn(()=>Promise.resolve({rowCount:1,rows:[{...release,active:false}]} as never));const service=createPrismaFormSchemaQueryService({execute,withTransaction:work=>work()},{authorize:()=>Promise.resolve({allowed:true,decisionId:"30000000-0000-4000-8000-000000000001"})});await expect(service.validateSubmission({context:requestContext,data:{synthetic_value:"blocked"},definitionId:"crm.synthetic.form",releaseVersion:1})).rejects.toMatchObject({code:"form_not_found"});});
  ...
  it("fails denial and contradictory assignment context before PostgreSQL", async () => {
    ...
    await expect(denied.getRelease({ context: requestContext, definitionId: "crm.synthetic.form", releaseVersion: 1 })).rejects.toMatchObject({ code: "form_denied" });
    // eslint-disable-next-line @typescript-eslint/unbound-method
    ...
    await expect(invalid.getRelease({ context: { ...requestContext, subject: { ...requestContext.subject, activeAssignmentIds: [] } }, definitionId: "crm.synthetic.form", releaseVersion: 1 })).rejects.toMatchObject({ code: "form_invalid_input" });
    // eslint-disable-next-line @typescript-eslint/unbound-method
  ...
  it("rejects accessor-backed request context without invoking it", async () => {
    ...
    await expect(service.getRelease({ context, definitionId: "crm.synthetic.form", releaseVersion: 1 }))
  ...
  it("rejects nested actor and assignment accessors without invoking them", async () => {
    ...
    await expect(service.getRelease({ context: { ...requestContext, actor } as FormQueryContext, definitionId: "crm.synthetic.form", releaseVersion: 1 }))
      .rejects.toMatchObject({ code: "form_invalid_input" });

    const assignments = [assignment];
    Object.defineProperty(assignments, "0", { enumerable: true, get: () => { reads += 1; return assignment; } });
    await expect(service.getRelease({ context: { ...requestContext, subject: { ...requestContext.subject, activeAssignmentIds: assignments } }, definitionId: "crm.synthetic.form", releaseVersion: 1 }))
      .rejects.toMatchObject({ code: "form_invalid_input" });
    expect(reads).toBe(0);
    // eslint-disable-next-line @typescript-eslint/unbound-method
    expect(db.execute).not.toHaveBeenCalled();

...
try {
  ...
  await waitForStablePostgres("ai_crm_auth_persist", "AUTH-PERSIST-01 PostgreSQL did not become ready.");
  ...
  bootstrapGlobalMigrations("ai_crm_auth_persist");
...
function run(command, arguments_, capture = false, environment = process.env) { const outcome = spawnSync(command, arguments_, { cwd: repositoryRoot, encoding: "utf8", env: environment, stdio: capture ? "pipe" : ["ignore", "ignore", "inherit"] }); if (outcome.status !== 0) throw new Error(`${command} failed with ${String(outcome.status)}.`); return outcome.stdout ?? ""; }

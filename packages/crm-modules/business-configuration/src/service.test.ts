...
const numberDefinition = (overrides: Partial<Omit<ParameterDefinition, "definitionVersion">> = {}): Omit<ParameterDefinition, "definitionVersion"> => ({
  ...
  parameterKey: "crm.synthetic.limit",
...
describe("business configuration service", () => {
  it("publishes immutable dictionary history and prevents stable code removal", async () => {
    ...
    const first = await instance.saveDictionaryDraft({ ...meta(), dictionaryId: "crm.synthetic.options", expectedRevision: 0, items: [{ code: "alpha", enabled: true, label: "Synthetic Alpha", order: 1 }], ownerModule: "crm.synthetic" });
    expect(first.draft.revision).toBe(1);
    await expect(instance.publishDictionary({ ...meta(), dictionaryId: "crm.synthetic.options", expectedRevision: 1 })).resolves.toMatchObject({ release: { releaseVersion: 1, version: 1 } });
    await instance.saveDictionaryDraft({ ...meta(), dictionaryId: "crm.synthetic.options", expectedRevision: 1, items: [{ code: "beta", enabled: true, label: "Synthetic Beta", order: 2 }], ownerModule: "crm.synthetic" });
    await expect(instance.publishDictionary({ ...meta(), dictionaryId: "crm.synthetic.options", expectedRevision: 2 })).rejects.toMatchObject({ code: "configuration_operation_conflict" });
    await expect(instance.getDictionaryRelease({ actor, dictionaryId: "crm.synthetic.options", releaseVersion: 1 })).resolves.toMatchObject({ items: [{ code: "alpha" }], releaseVersion: 1 });
  });
  ...
  it("validates typed values, rejects Secret-like strings and resolves an explicit default", async () => {
    ...
    await expect(instance.publishParameterValue({ ...meta(), parameterKey: "crm.synthetic.limit", value: 101 })).rejects.toMatchObject({ code: "configuration_invalid_input" });
    await expect(instance.resolveParameter({ actor, at: now, parameterKey: "crm.synthetic.limit", scopes: [] })).resolves.toEqual({ definitionVersion: 1, parameterKey: "crm.synthetic.limit", source: "default", value: 5, valueVersion: 0, version: 1 });

    await instance.registerParameter({ ...meta(), definition: { ...numberDefinition({ defaultValue: "safe", parameterKey: "crm.synthetic.label", valueSchema: { $schema: "https://json-schema.org/draft/2020-12/schema", maxLength: 40, type: "string" }, valueType: "string" }) } });
    await expect(instance.publishParameterValue({ ...meta(), parameterKey: "crm.synthetic.label", value: "Bearer abc123" })).rejects.toMatchObject({ code: "configuration_invalid_input" });
  });

  it("allows non-secret session policy identifiers while rejecting secret configuration identifiers", async () => {
    const instance = service();
    const parameterKey = "crm.authentication.pc-session.concurrent-limit";
    ...
    await expect(instance.registerParameter({ ...meta(), definition: numberDefinition({ parameterKey: "crm.authentication.client-secret" }) }))
  ...
  it("uses the highest declared scope priority and rejects overlapping intervals", async () => {
    ...
    await instance.publishParameterValue({ ...meta(), parameterKey: "crm.synthetic.limit", value: 10 });
    await instance.publishParameterValue({ ...meta(), parameterKey: "crm.synthetic.limit", value: 20 });
    ...
    await instance.activateParameter({ ...meta(), activationId: randomUUID(), effectiveFrom: "2026-07-01T00:00:00.000Z", parameterKey: "crm.synthetic.limit", scope: primaryScope, valueVersion: 2 });
    await instance.activateParameter({ ...meta(), activationId: randomUUID(), effectiveFrom: "2026-07-01T00:00:00.000Z", parameterKey: "crm.synthetic.limit", scope: secondaryScope, valueVersion: 1 });
    await expect(instance.resolveParameter({ actor, at: now, parameterKey: "crm.synthetic.limit", scopes: [secondaryScope, primaryScope] })).resolves.toMatchObject({ scope: primaryScope, source: "activation", value: 20, valueVersion: 2 });
    await expect(instance.activateParameter({ ...meta(), activationId: randomUUID(), effectiveFrom: "2026-07-15T00:00:00.000Z", parameterKey: "crm.synthetic.limit", scope: primaryScope, valueVersion: 1 })).rejects.toMatchObject({ code: "configuration_overlap" });
  });
  ...
  it("fails closed on missing values and replays only identical operation payloads", async () => {
    ...
    await expect(instance.registerParameter({ ...command, definition: { ...command.definition, ownerModule: "crm.changed" } })).rejects.toMatchObject({ code: "configuration_operation_conflict" });
    await expect(instance.resolveParameter({ actor, at: now, parameterKey: "crm.synthetic.limit", scopes: [] })).rejects.toMatchObject({ code: "configuration_missing" });
  });
  ...
  it("falls back from cache failures and retries invalidation failures", async () => {
    ...
    await instance.registerParameter({ ...meta(), definition: numberDefinition() });
    await expect(instance.resolveParameter({ actor, at: now, parameterKey: "crm.synthetic.limit", scopes: [] })).resolves.toMatchObject({ source: "default", value: 5 });
    failingCache.get.mockResolvedValue({ parameterKey: "different.parameter", source: "default", value: 99, version: 1 });
    await expect(instance.resolveParameter({ actor, at: now, parameterKey: "crm.synthetic.limit", scopes: [] })).resolves.toMatchObject({ parameterKey: "crm.synthetic.limit", value: 5 });
    failingCache.get.mockResolvedValue({ activationId: randomUUID(), definitionVersion: 1, effectiveFrom: "2027-01-01T00:00:00.000Z", parameterKey: "crm.synthetic.limit", scope: { scopeReference: "not-requested", scopeType: "context.primary" }, source: "activation", value: 99, valueVersion: 99, version: 1 });
    await expect(instance.resolveParameter({ actor, at: now, parameterKey: "crm.synthetic.limit", scopes: [{ scopeReference: "requested", scopeType: "context.primary" }] })).resolves.toMatchObject({ source: "default", value: 5 });
    failingCache.get.mockResolvedValue({ activationId: randomUUID(), definitionVersion: 1, effectiveFrom: "2026-07-01T00:00:00.000Z", parameterKey: "crm.synthetic.limit", scope: { scopeReference: "requested", scopeType: "context.primary" }, source: "activation", value: 99, valueVersion: 99, version: 1 });
    await expect(instance.resolveParameter({ actor, at: now, parameterKey: "crm.synthetic.limit", scopes: [{ scopeReference: "requested", scopeType: "context.primary" }] })).resolves.toMatchObject({ source: "default", value: 5 });
    failingCache.invalidate.mockRejectedValueOnce(new Error("cache unavailable")).mockResolvedValueOnce(undefined);
    const event = { eventId: randomUUID(), eventType: "configuration.parameter.published" as const, occurredAt: now, resourceId: "crm.synthetic.limit", version: 1 };

...
describe("configuration mutation authorization order", () => {
  it("denies before dictionary and parameter resource lookups", async () => {
    ...
    await expect(service.saveDictionaryDraft({ ...metadata(), dictionaryId: "crm.synthetic.options", expectedRevision: 1, items: [], ownerModule: "crm.synthetic" })).rejects.toMatchObject({ code: "configuration_denied" });
    await expect(service.publishDictionary({ ...metadata(), dictionaryId: "crm.synthetic.options", expectedRevision: 1 })).rejects.toMatchObject({ code: "configuration_denied" });
    await expect(service.publishParameterValue({ ...metadata(), parameterKey: "crm.synthetic.limit", value: 1 })).rejects.toMatchObject({ code: "configuration_denied" });
    await expect(service.activateParameter({ ...metadata(), activationId: randomUUID(), effectiveFrom: "2026-07-01T00:00:00.000Z", parameterKey: "crm.synthetic.limit", scope: { scopeReference: "synthetic:1", scopeType: "context.synthetic" }, valueVersion: 1 })).rejects.toMatchObject({ code: "configuration_denied" });
    await expect(service.terminateParameterActivation({ ...metadata(), activationId: randomUUID(), effectiveTo: "2026-07-15T00:00:00.000Z", parameterKey: "crm.synthetic.limit", terminationId: randomUUID() })).rejects.toMatchObject({ code: "configuration_denied" });

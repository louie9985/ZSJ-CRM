...
suite("PostgreSQL business configuration", () => {
  ...
  it("atomically publishes and resolves versioned values with provenance", async () => {
    ...
    const parameterKey = `crm.synthetic.${randomUUID().replaceAll("-", "")}`;
...
const definition = (parameterKey: string): Omit<ParameterDefinition, "definitionVersion"> => ({ allowedScopes: [{ priority: 1, scopeType: "context.synthetic" }], missingPolicy: "fail_closed", ownerModule: "crm.synthetic", parameterKey, valueSchema: { $schema: "https://json-schema.org/draft/2020-12/schema", maximum: 100, minimum: 0, type: "integer" }, valueType: "integer" });

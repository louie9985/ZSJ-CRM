...
const definition: Omit<ParameterDefinition, "definitionVersion"> = { allowedScopes: [{ priority: 1, scopeType: "context.synthetic" }], missingPolicy: "fail_closed", ownerModule: "crm.synthetic", parameterKey: "crm.synthetic.timeline", valueSchema: { $schema: "https://json-schema.org/draft/2020-12/schema", maximum: 100, minimum: 0, type: "integer" }, valueType: "integer" };
...
describe("immutable activation termination timeline", () => {
  it("terminates an open activation, preserves history and permits reactivation", async () => {
    ...
    expect(authorize).toHaveBeenCalledWith(expect.objectContaining({ action: "configuration:activate", ownerModule: "crm.synthetic" }));

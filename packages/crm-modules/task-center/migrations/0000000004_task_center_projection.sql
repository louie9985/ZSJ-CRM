create schema if not exists crm_task_center;

create table crm_task_center.task_projections (
...
create index task_projections_status_source_idx on crm_task_center.task_projections (status,source_type,source_task_id);

create table crm_task_center.projection_events (
...
create index projection_events_source_idx on crm_task_center.projection_events (source_type,source_task_id,source_version);

create table crm_task_center.task_commands (
...
comment on schema crm_task_center is 'Task Center owned projection and idempotent source-command routing state.';

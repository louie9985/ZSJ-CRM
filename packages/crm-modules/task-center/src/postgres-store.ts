...
class PrismaTaskCenterStore implements TaskCenterStore {
  ...
  private async readProjection(key:TaskProjectionKey,signal?:AbortSignal):Promise<TaskProjection|undefined>{
    const result=await this.execute<ProjectionRow>("select * from crm_task_center.task_projections where source_type=$1 and source_task_id=$2",[key.sourceType,key.sourceTaskId],signal);
  ...
  public async apply(event:TaskLifecycleEvent,signal?:AbortSignal):Promise<ProjectionApplyResult>{
    ...
    return await this.db.withTransaction(async()=>{
      ...
      const receipt=await this.execute<{payload_sha256:string}>("select payload_sha256 from crm_task_center.projection_events where event_id=$1",[event.eventId],signal);
      if(receipt.rows[0]){

...
Consumers exchange stable `FileReference` values. They never receive COS buckets, object keys, credentials, permanent URLs, SDK objects, or direct database access. Consumers may resolve trusted metadata only in the context of an active `ResourceLink`: resolution requires the complete reference and owning resource, independently authorizes and audits `file:reference`, and succeeds only while that immutable content version is `available`. It never exposes metadata from a bare content-version identifier.

`resolveFileReference` is an additive service capability, but its `file:reference` authorization action is intentionally fail-closed. Existing composition roots remain source-compatible and continue denying the new action until their reviewed permission mapping explicitly enables it; callers must not substitute `file:download` or `file:link` to bypass that review.
...
- `pnpm --filter @ai-crm/crm-file-center test`
- `pnpm --filter @ai-crm/crm-file-center test:integration`
- `pnpm --filter @ai-crm/crm-file-center typecheck`
- `pnpm --filter @ai-crm/crm-file-center lint`

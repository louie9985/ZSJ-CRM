...
const create = (service: ReturnType<typeof setup>["service"], operationId = randomUUID()) => service.createUploadSession({ ...meta(), declaredMediaType: "text/plain", declaredSizeBytes: 7, displayName: "synthetic.txt", operationId, ownerModule: "crm.synthetic" });
...
describe("File Center service", () => {
  ...
  it("makes a clean immutable version available, links it, and reauthorizes each download", async () => {
    ...
    const resource = { resourceId: "synthetic:1", resourceType: "crm.resource" }; const linked = await runtime.service.linkResource({ ...meta(), fileReference: created.fileReference, linkId: randomUUID(), ownerModule: "crm.synthetic", relationType: "crm.attachment", resource });
  ...
  it("issues a trusted FileReference only after malware scanning makes the version available", async () => {
    ...
    const resource = { resourceId: "synthetic:reference", resourceType: "crm.resource" };
    await runtime.service.linkResource({ ...meta(), fileReference: created.fileReference, linkId: randomUUID(), ownerModule: "crm.synthetic", relationType: "crm.attachment", resource });
    ...
    expect(runtime.authorizer.authorize).toHaveBeenCalledWith({ action: "file:reference", actor, resourceReference: "crm.resource:synthetic:reference" });
    expect(runtime.authorizer.authorize).toHaveBeenCalledWith({ action: "file:reference", actor, ownerModule: "crm.synthetic", resourceReference: "crm.resource:synthetic:reference" });
    expect(runtime.audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: "file:reference", resourceReference: "crm.resource:synthetic:reference", result: "succeeded" }));
    await expect(runtime.service.resolveFileReference({ ...meta(), fileReference: created.fileReference, resource: { resourceId: "synthetic:other", resourceType: "crm.resource" } })).rejects.toMatchObject({ code: "file_center_denied" });
    expect(runtime.audit.record).toHaveBeenCalledWith(expect.objectContaining({ action: "file:reference", resourceReference: "crm.resource:synthetic:other", result: "denied" }));
  ...
  it("quarantines malicious content and never signs a download", async () => {
    ...
    await expect(runtime.service.linkResource({ ...meta(), fileReference: created.fileReference, linkId: randomUUID(), ownerModule: "crm.synthetic", relationType: "crm.attachment", resource: { resourceId: "synthetic:1", resourceType: "crm.resource" } })).rejects.toMatchObject({ code: "file_center_not_ready" }); expect(runtime.storage.createDownloadGrant).not.toHaveBeenCalled();
  });
  ...
  it("requires authorization for both the owning resource and the file owner before linking", async () => {
    ...
    await expect(runtime.service.linkResource({ ...meta(), fileReference: created.fileReference, linkId: randomUUID(), ownerModule: "crm.other", relationType: "crm.attachment", resource: { resourceId: "synthetic:1", resourceType: "crm.resource" } })).rejects.toMatchObject({ code: "file_center_denied" }); expect(storeCall).not.toHaveBeenCalled(); expect(runtime.audit.record).toHaveBeenCalledWith(expect.objectContaining({ result: "denied" }));
  });
  ...
  it("audits a download denied by a missing current resource link", async () => {
    ...
    await expect(runtime.service.authorizeDownload({ ...meta(), fileReference: created.fileReference, resource: { resourceId: "synthetic:missing", resourceType: "crm.resource" } })).rejects.toMatchObject({ code: "file_center_denied" }); expect(runtime.audit.record).toHaveBeenCalledWith(expect.objectContaining({ result: "denied" })); expect(runtime.storage.createDownloadGrant).not.toHaveBeenCalled();
  });

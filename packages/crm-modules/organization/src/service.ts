...
export class OrganizationService implements OrganizationServiceApi {
  ...
  async #close(command: CloseEffectiveFactCommand, kind: "close_assignment" | "close_employment" | "close_organization_unit_placement", auditAction: string, eventType: string): Promise<void> {
    this.#metadata(command);

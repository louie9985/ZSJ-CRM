...
const useCase: AiUseCaseRegistration = {
  ...
  promptPolicyVersion: "prompt-reference.v1", proposalTtlMs: 60_000, requiresHumanConfirmation: true, useCaseId: "crm.synthetic.proposal", version: 1,
};
...
describe("AI gateway fake", () => {
  ...
  it("rejects unregistered and runtime-invalid use cases", async () => {
    const { service } = setup();
    await expect(service.invoke({ ...metadata(), useCaseId: "crm.unregistered" })).rejects.toMatchObject({ code: "ai_use_case_unavailable" });

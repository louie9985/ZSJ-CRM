...
class PrismaNotificationStore implements NotificationStore {
  ...
  public async getTemplateDefinition(templateKey: string): Promise<NotificationTemplateDefinition | undefined> {
    const result = await this.db.execute<DefinitionRow>("select * from platform_notifications.template_definitions where template_key=$1", [templateKey]);
  ...
  public async listTemplateDefinitions(): Promise<readonly NotificationTemplateDefinition[]> {
    const result = await this.db.execute<DefinitionRow>("select * from platform_notifications.template_definitions order by template_key");
  ...
  public saveTemplateDraft(input: { readonly draft: Omit<NotificationTemplateDraft, "revision">; readonly expectedRevision: number; readonly operationId: string }): Promise<NotificationTemplateDraft> {
    return this.db.withTransaction(async () => {
      ...
      const receipt = await this.db.execute<DraftOperationRow>("select * from platform_notifications.template_draft_operations where operation_id=$1", [input.operationId]);
      ...
      const result = await this.db.execute<DraftRow>(`insert into platform_notifications.template_drafts (template_key,revision,title_template,summary_template,body_template,updated_at) values ($1,1,$2,$3,$4,$5) on conflict (template_key) do update set revision=platform_notifications.template_drafts.revision+1,title_template=excluded.title_template,summary_template=excluded.summary_template,body_template=excluded.body_template,updated_at=excluded.updated_at where platform_notifications.template_drafts.revision=$6 returning *`, [input.draft.templateKey, input.draft.titleTemplate, input.draft.summaryTemplate, input.draft.bodyTemplate, input.draft.updatedAt, input.expectedRevision]);
      const row = result.rows[0];
      if (!row || row.revision !== input.expectedRevision + 1) throw new NotificationError("NOTIFICATION_CONFLICT");
      await this.db.execute("insert into platform_notifications.template_draft_operations (operation_id,template_key,expected_revision,revision,title_template,summary_template,body_template,updated_at) values ($1,$2,$3,$4,$5,$6,$7,$8)", [input.operationId, row.template_key, input.expectedRevision, row.revision, row.title_template, row.summary_template, row.body_template, row.updated_at]);
      return draft(row);
    });
  }

  public async getTemplateDraft(templateKey: string): Promise<NotificationTemplateDraft | undefined> {
    const result = await this.db.execute<DraftRow>("select * from platform_notifications.template_drafts where template_key=$1", [templateKey]);
  ...
  public async getTemplate(key: string, version: number): Promise<TemplateRelease | undefined> {
    const result = await this.db.execute<TemplateRow>("select * from platform_notifications.template_releases where template_key=$1 and version=$2", [key, version]);
  ...
  public async listTemplateReleases(templateKey: string): Promise<readonly TemplateRelease[]> {
    const result = await this.db.execute<TemplateRow>("select * from platform_notifications.template_releases where template_key=$1 order by version desc", [templateKey]);
  ...
  public activateTemplate(input: { readonly activatedAt: string; readonly activationId: string; readonly templateKey: string; readonly version: number }): Promise<void> {
    return this.db.withTransaction(async () => {
      ...
      const inserted = await this.db.execute<{ activation_id: string; template_key: string; version: number }>(`insert into platform_notifications.template_activation_history (activation_id,template_key,version,activated_at) values ($1,$2,$3,$4) on conflict (activation_id) do nothing returning activation_id,template_key,version`, [input.activationId, input.templateKey, input.version, input.activatedAt]);
      if (!inserted.rows[0]) {
        const existing = await this.db.execute<{ template_key: string; version: number }>("select template_key,version from platform_notifications.template_activation_history where activation_id=$1", [input.activationId]);
      ...
      await this.db.execute(`insert into platform_notifications.current_template_release (template_key,version,activation_id,activated_at) values ($1,$2,$3,$4) on conflict (template_key) do update set version=excluded.version,activation_id=excluded.activation_id,activated_at=excluded.activated_at`, [input.templateKey, input.version, input.activationId, input.activatedAt]);
    });
  }

  public async getActiveTemplate(templateKey: string): Promise<TemplateRelease | undefined> {
    const result = await this.db.execute<TemplateRow>(`select r.* from platform_notifications.current_template_release c join platform_notifications.template_releases r on r.template_key=c.template_key and r.version=c.version where c.template_key=$1`, [templateKey]);
    return result.rows[0] ? template(result.rows[0]) : undefined;
  }

  public async findIntent(producer: string, key: string): Promise<StoredIntent | undefined> {
    const result = await this.db.execute<IntentRow>("select * from platform_notifications.notification_intents where producer=$1 and idempotency_key=$2", [producer, key]);
  ...
  public async get(principalId: string, id: string): Promise<InAppNotification | undefined> {
    const result = await this.db.execute<NotificationRow>("select * from platform_notifications.in_app_notifications where principal_id=$1 and notification_id=$2 and preference_decision='deliver'", [principalId, id]);
  ...
  public async list(input: { readonly principalId: string; readonly limit: number; readonly cursor?: string; readonly includeArchived: boolean }): Promise<NotificationPage> {
    ...
    const result = await this.db.execute<NotificationRow>(`select * from platform_notifications.in_app_notifications where principal_id=$1 and preference_decision='deliver' and ($2::boolean or archived_at is null) and ($3::timestamptz is null or (created_at,notification_id)<($3::timestamptz,$4::uuid)) order by created_at desc,notification_id desc limit $5`, [input.principalId, input.includeArchived, cursorAt, cursorId, input.limit + 1]);

...
const container = `ai-crm-plt01-audit-${randomUUID().slice(0, 8)}`;
const pnpmCli=process.env.npm_execpath;if(!pnpmCli)throw new Error("pnpm execution path is unavailable.");
const directory = await mkdtemp(join(tmpdir(), "ai-crm-plt01-audit-"));
...
try {
  ...
  run("docker", ["run", "--detach", "--name", container, "--publish", "127.0.0.1::5432", "--env", "POSTGRES_USER=ai_crm_migration", "--env", "POSTGRES_DB=ai_crm_plt01", "--env", "POSTGRES_PASSWORD_FILE=/run/secrets/postgres_password", "--mount", `type=bind,source=${resolve(passwordFile)},target=/run/secrets/postgres_password,readonly`, "postgres:17.5-alpine"]);
  await waitForStablePostgres("ai_crm_plt01", "PLT-01 audit PostgreSQL did not become ready.");
  const port = run("docker", ["port", container, "5432/tcp"], true).trim().split(":").at(-1); if (!port || !/^\d+$/u.test(port)) throw new Error("PLT-01 audit PostgreSQL port is unavailable.");
  await writeFile(urlFile, `postgresql://ai_crm_migration:${password}@127.0.0.1:${port}/ai_crm_plt01\n`, { encoding: "utf8", mode: 0o600 }); await chmod(urlFile, 0o600);
  bootstrapGlobalMigrations("ai_crm_plt01");
  const result = spawnSync(process.execPath, [resolve(import.meta.dirname, "../../../../node_modules/vitest/vitest.mjs"), "run", "--config", "../../../vitest.config.ts", "src/postgres-store.integration.test.ts"], { cwd: resolve(import.meta.dirname, ".."), env: { ...process.env, TEST_AUDIT_DATABASE_URL_FILE: urlFile }, stdio: "inherit" });
  if (result.status !== 0) throw new Error(`PLT-01 audit integration failed with ${String(result.status)}.`);
} catch (error) { primaryError = error; } finally { const removed = spawnSync("docker", ["rm", "--force", "--volumes", container], { encoding: "utf8" }); if (removed.status !== 0 && !removed.stderr.includes("No such container")) cleanupError = new Error("PLT-01 audit PostgreSQL cleanup failed."); try { await rm(directory, { force: true, recursive: true }); } catch { cleanupError ??= new Error("PLT-01 audit Secret cleanup failed."); } }
if (primaryError && cleanupError) throw new AggregateError([primaryError, cleanupError], "PLT-01 audit integration and cleanup failed."); if (primaryError) throw primaryError; if (cleanupError) throw cleanupError;

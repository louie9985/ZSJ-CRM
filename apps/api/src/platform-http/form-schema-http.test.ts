...
const release = Object.freeze({
  active: true, contentDigest: "a".repeat(64), definitionId: "crm.synthetic", jsonSchema: {}, ownerModule: "crm.synthetic",
...
function request(overrides: Partial<FormSchemaHttpRequest> = {}): FormSchemaHttpRequest {
  return { at: "2026-07-28T00:00:00.000Z", credential: "opaque-session", method: "GET", path: "/form-definitions/crm.synthetic/releases/2", ...overrides };
}
...
describe("createFormSchemaHttpAdapter", () => {
  it("reads one exact release and maps authorized identity and permission", async () => {
    ...
    expect(target.authorize).toHaveBeenCalledWith({ at: "2026-07-28T00:00:00.000Z", credential: "opaque-session", permission: { action: "read", resource: "crm.form-schema.form-release" }, selectedAssignmentId: assignmentId });
  ...
  it("validates a strict JSON request and invokes the validation query", async () => {
    const target = fixture();
    const result = await target.adapter.handle(request({ body: "{\"data\":{\"value\":1}}", contentType: "application/json; charset=utf-8", method: "POST", path: "/form-definitions/crm.synthetic/releases/2/validate" }));

    expect(result.status).toBe(200);
    expect(target.authorize).toHaveBeenCalledWith(expect.objectContaining({ permission: { action: "validate", resource: "crm.form-schema.form-release" } }));
    expect(target.validateSubmission).toHaveBeenCalledWith(expect.objectContaining({ data: { value: 1 }, definitionId: "crm.synthetic", releaseVersion: 2 }));
    expect(target.calls).toEqual(["authorize", "service"]);
  });

  it.each([
    ["query", { path: "/form-definitions/crm.synthetic/releases/2?current=true" }],
    ["encoded path", { path: "/form-definitions/platform%2Esynthetic/releases/2" }],
    ["leading-zero version", { path: "/form-definitions/crm.synthetic/releases/02" }],
    ["unsafe version", { path: `/form-definitions/crm.synthetic/releases/${String(Number.MAX_SAFE_INTEGER + 1)}` }],
  ...
  it.each([
    ...
    const target = fixture();
    const result = await target.adapter.handle(request({ method: "POST", path: "/form-definitions/crm.synthetic/releases/2/validate", ...override }));
  ...
  it("enforces 256 KiB by encoded UTF-8 bytes before authorization", async () => {
    ...
    const result = await target.adapter.handle(request({ body, contentType: "application/json", method: "POST", path: "/form-definitions/crm.synthetic/releases/2/validate" }));
  ...
  it("enforces data depth 32 before authorization", async () => {
    ...
    expect((await accepted.adapter.handle(request({ body: JSON.stringify({ data: boundary }), contentType: "application/json", method: "POST", path: "/form-definitions/crm.synthetic/releases/2/validate" }))).status).toBe(200);
    const target = fixture();
    let data: unknown = null;
    for (let depth = 0; depth < 33; depth += 1) data = [data];
    const result = await target.adapter.handle(request({ body: JSON.stringify({ data }), contentType: "application/json", method: "POST", path: "/form-definitions/crm.synthetic/releases/2/validate" }));
    expect(result.status).toBe(413);
    expect(target.calls).toEqual([]);
  });
  ...
  it("counts object, array, and scalar data nodes including the root", async () => {
    ...
    expect((await accepted.adapter.handle(request({ body: JSON.stringify({ data: exactlyTenThousand }), contentType: "application/json", method: "POST", path: "/form-definitions/crm.synthetic/releases/2/validate" }))).status).toBe(200);
    const rejected = fixture();
    const tenThousandAndOne = Array.from({ length: 10_000 }, () => null);
    expect((await rejected.adapter.handle(request({ body: JSON.stringify({ data: tenThousandAndOne }), contentType: "application/json", method: "POST", path: "/form-definitions/crm.synthetic/releases/2/validate" }))).status).toBe(413);
    expect(rejected.calls).toEqual([]);
  });

  it("rejects absent credentials before authorization", async () => {
    const target = fixture();
    expect((await target.adapter.handle({ at: "2026-07-28T00:00:00.000Z", method: "GET", path: "/form-definitions/crm.synthetic/releases/2" })).status).toBe(401);
    expect(target.calls).toEqual([]);
  });
  ...
  it.each([
    [new BrowserSessionFailure("authentication_required"), 401, "form_unauthorized"],
  ...
  it("maps module validation errors after authorization without retrying", async () => {
    ...
    const result = await target.adapter.handle(request({ body: "{\"data\":null}", contentType: "application/json", method: "POST", path: "/form-definitions/crm.synthetic/releases/2/validate" }));
    expect(result.status).toBe(400);
